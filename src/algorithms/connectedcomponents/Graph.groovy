...
public class Graph {
	...
	public Map<Vertex, Set<Vertex>> kosaraju() {
	...
	void dfs(Vertex i) {
		...
		while (!dfsStack.isEmpty()){
			...
			v.leader = s
			Set<Edge> vEdges = this.edges.get(v)
			if (vEdges != null) {
				for (Edge vj : vEdges) {
					Vertex j = vj.to
	...
	void transposeDFS(Vertex i) {
		...
		while (!dfsStack.isEmpty()){
			Vertex v = dfsStack.pop()
			this.exploredVertices.add(v)
			Set<Edge> vEdges = this.reverseEdges.get(v)
			if (vEdges != null) {
				for (Edge vj : vEdges) {
					Vertex j = vj.to
					boolean isJExplored = this.exploredVertices.contains(j)
			...
			this.finishingTimes++
			v.finishingTime = this.finishingTimes
		}
	}
	...
	public void readFromFile(String sourceFile) throws IOException {
